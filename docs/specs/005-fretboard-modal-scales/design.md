...
const MODAL_CHARACTERISTIC_INTERVAL: Partial<Record<ScaleType, number[]>> = {
  ...
  lydian: [6], // #4(ラベル表示も ♯4、PR レビュー後) — メジャーの 4 との差
...
- **model**(`src/model/theory.test.ts`、テストの主戦場):
  - `isCharacteristicTone` に `describe` ブロックを追加: フリジアン(♭2 のみ true)、リディアン(増4度 = 半音 6 のみ true、ラベルが `♯4` になることも確認)、ロクリアン(♭2 と♭5 の両方が true、それ以外は false)
...
- **手動確認**(`npm run dev`):
  ...
  2. 「リディアン」→ 増4度の位置が紫になり、ラベル表示「度数」で `♯4` と表示されることを確認
...
| AC-3 | `theory.test.ts` リディアン特徴音テスト(度数ラベルが `♯4` になることを含む)+ 手動確認 2 |
