...
- **ラベル切替(AC-3, 11)**: 3 ボタンのセグメント(既存 `STYLE_LABELS` と同じ btn パターン)。keyContext 設定中のみ表示。クリックで `labelMode` を保存。「なし」の UI 表記は **「点のみ」**(スケール select の「なし(自動表示オフ)」との意味衝突を避ける。内部値は `'none'`)。♯/♭ トグルは labelMode が「音名」のときのみ表示し、`keyContext.preferFlats` を反転
